...
def oscillate_motors(steps=10):

    print("""Oscillates the three motors back and forth for the given steps.""")
...
def process_order():
    ...
    while True:  
        ...
        try:
            ...
            oscillate_thread1 = threading.Thread(target=blending_process(water_quantity))
            oscillate_thread2 = threading.Thread(target=oscillate_motors)

            # Start both threads
            oscillate_thread1.start()
            oscillate_thread2.start()

            # Wait for both threads to complete
            oscillate_thread1.join()
            oscillate_thread2.join()
